...
public class GameMap implements Serializable {
        ...
        for (int y = 0; y < list.size(); y++) {
            for (int x = 0; x < list.get(y).length(); x++) {
                ...
                for (int i = 0; i < CellType.values().length; i++) {
                    if (currentSymb != CellType.values()[i].datSymbol) continue;
                    data[x][mapSizeY - y - 1] = CellType.values()[i];
                    if (CellType.values()[i] == CellType.FOOD) foodCount++;
    ...
    public boolean isCellEmpty(int cellX, int cellY) {
        return cellX < 0 || cellX >= mapSizeX || data[cellX][cellY] != CellType.WALL;
    }
