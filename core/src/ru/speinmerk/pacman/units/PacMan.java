package ru.speinmerk.pacman.units;
...
import ru.speinmerk.pacman.Assets;
import ru.speinmerk.pacman.GameMap;
import ru.speinmerk.pacman.GameScreen;
...
public class PacMan extends Unit implements Serializable {
    ...
    public void update(float dt) {
        ...
        if (Vector2.dst(position.x, position.y, destination.x, destination.y) < 0.001f) {
            if (position.x < 0) {
                position.x = gameMap.getMapSizeX();
                destination.set(position);
                move(Direction.LEFT, false);
                return;
            }
            if (position.x > gameMap.getMapSizeX() - 1) {
                position.x = -1;
                destination.set(position);
